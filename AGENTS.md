...
touching any `data/*.json` — plus bootstrap-project, data-edit, mine-video,
dossier-research); the architecture rationale in `cronologia/fsp` → `docs/adrs/`.
On this machine the skills live at `/home/user/core/skills/` (e.g.
`/home/user/core/skills/sourcing-rules/SKILL.md` and
`/home/user/core/skills/data-edit/SKILL.md` — the mandatory
validate → test → build → commit-with-docs gate for every data edit).

## What this project is

A compiled static website documenting the chronology of **Nossa Senhora das
Graças / the Miraculous Medal** — the Marian apparitions reported by Catherine
Labouré at the rue du Bac chapel, Paris, in 1830, the medal struck in 1832,
and the Church judgments on both. A single JSON file is the source of truth;
a zero-dependency Node script compiles it into static HTML served by GitHub
Pages.
...
data/i18n/{es,pt}.json   HAND-AUTHORED translation dictionaries (exact English source string -> translation; committed). In THIS repo they were authored by hand, not by scripts/translate.js — keep them in sync when English changes and keep _meta.generatedBy honest
...
The five core rules of `/home/user/core/skills/sourcing-rules/SKILL.md` apply
verbatim. Subject-specific rules for this repo:

1. **Apparitions are REPORTED events with Church judgments.** Every apparition
   entry states who reported what and when (the attestation chain: Catherine
   Labouré → her confessor Aladel → her later written accounts), and what the
   Church ruled and when, citing the ruling document. The site's voice never
   asserts or denies the supernatural claim.
2. **Church-approval status is a critical field.** Keep the `Church approval
   status` fact precise: approval attached to the medal (1832 authorization),
   the feast (1894) and the canonization (1947). What the 1836 Paris inquiry
   concluded is CONTESTED between sources — devotional accounts say it declared
   the medal of supernatural origin; the detailed Vincentian-derived account
...
4. **Preferred primaries:** the chapel's own site
   (chapellenotredamedelamedaillemiraculeuse.com) and vatican.va. Devotional
   sources (amm.org, catechisms) are labeled sympathetic; Wikipedia entries are
   tertiary syntheses whose claims should be traced to their cited literature.
5. **Check `KEYWORDS.md` before any corpus/web sweep** — name variants in
   pt/es/fr/la/it live there, with the disambiguation traps ("Our Lady of
   Grace" ≠ this devotion by default; "Madonna del Miracolo" is the Ratisbonne
   image).
